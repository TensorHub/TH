...
export class Week {
    ...
    /**
     * Возвращает URL для PDF статьи
     */
    getPaperUrl(githubRepo, branch) {
        if (!this.paper_path) return null;
        return `https://raw.githubusercontent.com/${githubRepo}/${branch}/${this.paper_path}`;
    }
    ...
    getFormattedResources(githubRepo, branch) {
        ...
        if (this.papers !== undefined) {
            const paperText = this.paper_path ? 'Open Paper' : `${this.papers} Paper${this.papers !== 1 ? 's' : ''}`;
            const resource = {
                icon: 'fas fa-file-pdf',
                text: paperText,
                type: 'papers'
            };
            const url = this.getPaperUrl(githubRepo, branch);
            if (url) {
                resource.url = url;
            }
